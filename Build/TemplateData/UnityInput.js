...
cnvasParent = document.getElementById("webglContainer");
cnvas = document.getElementById("#canvas");
currTouchDiv = cnvas; //TODO: Check this
...
function fitCanvasToScreen(){
	...
	{
		var ratio = cnvasParent.clientHeight/cnvasParent.clientWidth;
		cnvasParent.style.width = window.innerWidth+'px';
		var parsedWidth = parseInt(cnvasParent.style.width, 10)
		cnvasParent.style.height = (parsedWidth * ratio)+'px';
	}
}
...
{
	//TODO: Validate fake landscape mode
	if(fullscreen)
	{
		canvasX = center.x;
	...
	{
		canvasX = center.x - cnvasParent.offsetLeft + (cnvasParent.clientWidth/2);
		canvasY = (cnvasParent.clientHeight - (center.y - cnvasParent.offsetTop))-window.scrollY;
	}
